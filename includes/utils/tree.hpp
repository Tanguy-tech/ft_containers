...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/05 09:22:37 by tbillon           #+#    #+#             */
/*   Updated: 2022/05/09 09:42:32 by tbillon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	...
	private:
		void _deepRemove(node_ptr z) //* Remove node
		{
			node_ptr y = z;
			node_ptr x;
		...
		void _removeFix(node_ptr x) //* Balance after removing node
		{
			node_ptr s;
		...
		node_ptr _findMin(node_ptr root) const //* Find min node value to be able to balance tree after operations
		{
		...
		node_ptr _findMax(node_ptr root) const //* Find max node value to be able to balance tree after operations
		{
		...
		void _leftRotate(node_ptr x) //* Left rotation to rebalance tree 
		{
			node_ptr y = x->right;
			x->right = y->left;
			if (y->left != _NIL)
		...
		void _rightRotate(node_ptr x) //* Right rotation to rebalance tree 
		{
			node_ptr y = x->left;
			x->left = y->right;
			if (y->right != _NIL)
